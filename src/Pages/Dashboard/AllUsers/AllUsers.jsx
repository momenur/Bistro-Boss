...
const AllUsers = () => {
  ...
  const handleUserDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://server-bisto.vercel.app/users/${id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
  ...
  const handleMakeAdmin = (id) => {
    fetch(`https://server-bisto.vercel.app/users/admin/${id}`, {
      method: "PATCH",
    })
      .then((res) => res.json())
      .then((data) => {
                ...
                <tr key={row._id}>
                    ...
                    {row.role === "admin" ? (
                      ...
                      <>
                        <button
                          ...
                          className="btn bg-[#D1A054] text-xl text-white p-3 hover:bg-yellow-500 hover:text-black"
                        >
                          <FaUserShield />
                        </button>
                      </>
                  ...
                  <th>
                    <button
                      ...
                      className="btn bg-[#B91C1C] text-xl text-white p-3 hover:bg-yellow-500 hover:text-[#B91C1C]"
                    >
                      <FaTrash />
                    </button>
                  </th>
